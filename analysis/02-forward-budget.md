# 2. Forward Budget — What Annual Spend Should Look Like, 2026–2035

*Separating "needs" (advocacy arithmetic) from "budgets" (what will actually be spent).*

---

## 2.1 The four published estimates, and why they disagree by 3x

Every number in this sector traces to one of four studies. They are not measuring the same
thing, which is why the range is so wide.

| Source | Published | Estimate | Annualised | What it actually measures |
| --- | --- | --- | --- | --- |
| **EPA 7th DWINSA** | Sep 2023 | $625bn / 20yr (Jan-2021 $); $648.8bn in 2022 $ | **$31.3bn/yr** | *SRF-eligible* drinking water capital projects reported by states |
| **EPA CWNS 2022** | Apr 2024 | $630.1bn / 20yr | **$31.5bn/yr** | *SRF-eligible* clean water capital needs with documentation |
| **ASCE Bridging the Gap** | 2024 | $99bn/yr *gap* (up from $81bn in 2021) | implies **~$187bn/yr required** | Full economic need incl. resilience, service quality, growth |
| **AWWA *Beyond the Replacement Era*** | Mar 2026 | $2.1–2.4tn / 25yr, drinking water **only** (2025 $) | **$84–96bn/yr** | Asset replacement **plus** PFAS, LSL, climate resilience, cyber, rising O&M |

### The reconciliation

- **The EPA surveys are floors, by construction.** They count only what states could
  document as eligible. EPA itself concedes the surveys "likely underestimate needs for
  transmission and distribution systems because many systems do not have a plan in place for
  replacing pipes." The DWINSA baseline assumes a **0.5%/year pipe replacement rate — which
  imputes a 200-year pipe life**, roughly four times the observed 53-year average failure age.
- **The ASCE and AWWA numbers are ceilings, also by construction.** They are produced by an
  engineers' professional society and a utility trade association respectively, both of
  which advocate for higher appropriations. AWWA's headline requires a **168% increase in
  annual capital investment** and concedes household bills would roughly double in real terms.
- **The truth is bracketed, not split.** Escalating the two EPA surveys to 2026 dollars at
  ~4.5%/yr construction inflation gives **~$76bn/yr of documented, eligible capital need**.
  Actual water-related construction put in place is running at **~$85–90bn/yr**. On a
  like-for-like basis the sector is roughly *meeting* its documented eligible need — and
  falling far short only against the broader definitions.

**The honest framing for an investor: there is no cliff-edge deficit, there is a slow
compounding one.** The market grows because rates rise and mandates bind, not because a
$99bn/year gap is about to be closed.

---

## 2.2 The market-research view (the one that maps to distributor revenue)

Bluefield Research's forecasts are the closest to a bottom-up, project-pipeline model:

| Forecast | Value | Implied annual |
| --- | --- | --- |
| US municipal water + wastewater **treatment** capex, through 2035 | **$515.4bn**, 4.4% CAGR | $37.2bn (2026) → **$57.3bn (2035)** |
| — of which wastewater | $310.4bn (58%) | |
| — of which upgrades/rehab of existing assets | **$406.4bn (79%)** | The replacement thesis, quantified |
| US + Canada municipal **pipe** capex, 2026–2035 | **$99.3bn** | ~$10bn/yr |
| Non-revenue water losses | 19.5% of treated water; $6.4bn/yr lost revenue | Self-funding replacement case |

The critical figure is **79% of forecast treatment spend going to rehabilitation of
existing assets rather than new build**. That is the single best confirmation that the
sector's growth is replacement-led, not capacity-led — which is exactly the mix a
distributor with a 50% repair-and-replace revenue base wants.

---

## 2.3 The federal picture through 2027

| Item | Status as of August 2026 |
| --- | --- |
| IIJA water supplemental (>$50bn, FY2022–26) | **Expires 30 September 2026** |
| Regular SRF appropriations FY2024/25/26 | **Flat** — no organic growth |
| FY2027 House Interior/Environment bill | **16% SRF cut** proposed; DWSRF to $910m from $1,126m |
| FY2027 earmarks | ~1,200 water projects, >$1bn — carved *out of* SRF, not added |
| Senate EPW **WRDA 2026** (text released 13 July 2026) | Would reauthorise **$14bn/4yr CWSRF**, **$16.5bn/5yr DWSRF**, WIFIA at $65m/yr through FY2030 |
| WIFIA capacity | ~$11bn available; $23bn deployed across 152 loans to date |

**Read WRDA 2026 carefully.** $16.5bn over five years for DWSRF is **$3.3bn/year — an
authorisation, not an appropriation**, and roughly triple the current $1.1bn appropriated
level but well below the ~$3.7bn/yr the DWSRF actually received with IIJA on top. Even in
the bullish legislative case, federal drinking water support settles *below* the 2022–26
run-rate. Authorisation levels are also routinely under-appropriated.

---

## 2.4 The IIJA cliff arithmetic, done honestly

This is the single most-asked question about CNM and FERG, and most commentary gets the
magnitude wrong in both directions.

**Step 1 — the gross federal step-down.** Per the National League of Cities, IIJA took
annual SRF funding from **~$2.7bn (FY2021) to ~$11.4bn (FY2022–26)**. Gross step-down:
**$8.7bn/year** — 9.9% of the ~$88bn national water construction base.

**Step 2 — the pipeline is not empty.** Core & Main's President, on the 10 June 2026
earnings call: *"the remaining funding is expected to hit the state revolving funds this
year… only about a third or less of it has hit the municipality level yet. There's still a
lot of funding out there for municipalities to use."* Roughly **two-thirds of ~$50bn — some
$33bn — is still to be disbursed**, spreading the drawdown across 2027–2030.

**Step 3 — a large share revolves.** SRFs are revolving loan funds. The general SRF pots are
predominantly loans; the LSL and emerging-contaminant pots carry high principal-forgiveness
requirements. On a rough 50/50 grant-vs-loan split, ~$25bn of permanent corpus addition
supports **~$2bn/year of ongoing lending in perpetuity**. Net steady-state loss:
**~$6.7bn/year, or 7.6% of the base** — not 100% of the supplemental.

**Step 4 — the offset.** Rate growth of 5.1%/yr on a much larger revenue base swamps this.
A single year of national rate increases generates more incremental utility capacity than
the entire annual IIJA water supplemental.

---

## 2.5 Build-up model: national water capital spending, 2027–2035

Assumptions stated explicitly so each can be stress-tested:

- **Base:** $88bn (2026 run-rate; Census C30 water supply + sewage/waste disposal, total construction, SAAR Jan-2026)
- **Organic growth:** 4.5%/yr nominal (Bluefield treatment capex 4.4% CAGR; rate growth ~5%)
- **IIJA drawdown:** net $6.7bn/yr at full effect, phased 15% / 40% / 75% / 100% across 2027–2030
- **LSL adder:** incremental spend under the LCRI 10-year window, peaking ~$5.5bn/yr around 2030–31
- **PFAS adder:** incremental capital + O&M above base, peaking ~$2.5bn/yr around 2029

| Year | Organic | IIJA drag | LSL | PFAS | **Total ($bn)** | YoY |
| --- | --- | --- | --- | --- | --- | --- |
| 2027 | 92.0 | −1.0 | 2.0 | 1.0 | **94.0** | +6.8% |
| 2028 | 96.1 | −2.7 | 3.5 | 1.8 | **98.7** | +5.1% |
| 2029 | 100.4 | −5.0 | 5.0 | 2.5 | **102.9** | +4.2% |
| 2030 | 104.9 | −6.7 | 5.5 | 2.5 | **106.2** | +3.2% |
| 2031 | 109.7 | −6.7 | 5.5 | 2.0 | **110.5** | +4.0% |
| 2032 | 114.6 | −6.7 | 5.0 | 1.5 | **114.4** | +3.6% |
| 2033 | 119.8 | −6.7 | 4.5 | 1.2 | **118.8** | +3.8% |
| 2034 | 125.1 | −6.7 | 3.5 | 1.0 | **122.9** | +3.5% |
| 2035 | 130.8 | −6.7 | 2.0 | 1.0 | **127.1** | +3.4% |

**Conclusion: roughly 3.5–5% nominal annual growth in national water capital spending
through 2035, with the federal drawdown shaving ~1–1.5 percentage points off growth in the
2029–2030 window and nothing thereafter.** This is a mid-single-digit market, not a boom
and not a bust. Cumulative 2027–2035 spend of **~$996bn** sits comfortably between the EPA
surveys' escalated floor and the ASCE/AWWA ceiling.

### Sensitivity

| Scenario | Change to assumptions | 2030 total | 2035 total |
| --- | --- | --- | --- |
| **Bear** — SRF cut 16% and sustained, no WRDA reauthorisation, rates flatten to 3% | growth 3.0%, drag $9bn | ~$97bn | ~$110bn |
| **Base** — as modelled | 4.5% growth, $6.7bn drag | **$106bn** | **$127bn** |
| **Bull** — WRDA 2026 appropriated in full, LCRI upheld in litigation, rates hold 5.5% | growth 5.5%, drag $4bn | ~$114bn | ~$146bn |

The spread between bear and bull by 2035 is about **$36bn/yr, or ~30%** — material, but the
*direction* is up in every case. There is no credible scenario in which national water
capital spending declines in nominal terms.

---

## 2.6 The two regulatory line items that are legally, not economically, driven

These are the highest-conviction components of the forecast because they are court-enforced
deadlines rather than budget preferences.

### Lead service line replacement

| Fact | Value |
| --- | --- |
| National lead service lines (EPA 7th DWINSA, first-ever count) | **9.2 million** |
| EPA cost per line | $4,700 – $6,930 (utilities dispute as low; NY estimates up to $11,000) |
| Implied national total | **$46bn – $101bn**; consensus estimates cluster at $45–60bn |
| Dedicated IIJA funding | $15bn — covers roughly a quarter to a third |
| LCRI compliance date | **1 November 2027** for baseline inventory, replacement plan, school/childcare list, updated sampling plan |
| Replacement mandate | All LSLs within **10 years** (2034–2037 depending on system), regardless of measured lead levels |
| **Litigation risk** | **AWWA v. EPA** — briefing complete, **oral argument expected Fall 2026** |

This is a ~$5bn/year materials-and-installation programme with a hard legal deadline and a
funding gap of $30–45bn that must come from rates. The AWWA litigation is the single largest
binary risk to the forecast; it challenges the mandated replacement of lines "under the
control" of the utility.

### PFAS

| Fact | Status (August 2026) |
| --- | --- |
| PFOA / PFOS MCLs at 4 ppt | **Retained** — EPA proposed 18 May 2026 to uphold them |
| Compliance deadline | Proposed extension from 26 April 2029 to **26 April 2031** on request (not automatic) |
| PFHxS, PFNA, HFPO-DA (GenX), Hazard Index | **Proposed for rescission** — on legal/procedural grounds under SDWA, not on the health science |
| Comment period | Closed 20 July 2026 |
| Sector cost (AWWA / Black & Veatch) | **>$50bn over 20 years** for treatment capital + O&M |
| CERCLA hazardous-substance designation | Could add **~$3.5bn/yr** in residuals disposal costs |
| Household rate impact | $305–$3,570/yr, worst for small systems |

Net effect of the 2026 proposals: **the PFAS opportunity is smaller and later than the 2024
rule implied**. Retaining only PFOA/PFOS and slipping compliance two years reduces the
number of systems triggered and pushes capital into 2029–2031. Treat any model built on the
full 2024 six-compound rule as stale.

---

## 2.7 What this means for the distributors

Translating the national model into revenue expectations:

- **Market growth of 3.5–5% nominal** is the ceiling for organic volume+price growth in
  waterworks distribution, absent share gains.
- **Core & Main's fiscal 2026 guidance of $7.8–7.9bn on $7.647bn (2.0–3.3% growth)** sits
  *below* that, consistent with residential softness (18% of its mix) offsetting municipal strength.
- **Above-market growth must come from share, M&A, and adjacent product lines** — treatment
  plant, metering/AMI, private label — not from the market itself.
- **The 2029–2030 window is where the federal drag concentrates.** If a slowdown appears, it
  should appear there, and it should be worth roughly 100–150bps of market growth, not more.

---

**Next:** [3. Technical Drivers — why replacement is actually needed](03-technical-drivers.md)

*Company guidance and management commentary sourced from Quartr.*
